// dev server over public/. watches src/, rebuilds on change, pings the
// browser via SSE so it can reload. injects a tiny client into served
// html only; the production build never sees it.
...
try {
  const { counts } = build()
  console.log(`built  functions=${counts.functions}  divergences=${counts.divergences}`)
} catch (e) {
  console.error("build failed:", e.message)
}

// debounced rebuild on src/ change
...
createServer((req, res) => {
  ...
  console.log(`serving ${OUT} at http://localhost:${PORT}  (watching ${SRC})`)
})
