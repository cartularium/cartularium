...
// formulas driver — drives the pure-python `formulas` xlsx evaluator
// (https://github.com/vinci1it2000/formulas) via formulas_driver.py
// stages tasks as one xlsx, sheet per task, then `formulas` calcs the workbook

const PROJECT_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const PYTHON_SCRIPT = join(PROJECT_ROOT, "python", "formulas_driver.py");

export class FormulasDriver implements Driver {
  readonly platform = "formulas" as const;
  private tmpDir: string | null = null;

  async init(): Promise<void> {
    try {
      execSync("uv run python -c 'import formulas'", {
        cwd: PROJECT_ROOT,
        stdio: "pipe",
      });
    } catch {
      throw new Error("`formulas` package not available. Run: assay setup");
    }
    this.tmpDir = mkdtempSync(join(tmpdir(), "assay-formulas-"));
  }
  ...
  async evaluate(formula: string, grid?: Record<string, CellValue>): Promise<RichGridValue> {
    ...
    try {
      ...
      throw new Error(`formulas evaluation failed: ${err.stderr?.toString() || String(e)}`);
    }
    const raw = JSON.parse(readFileSync(resultFile, "utf8"));
    if (raw.error) throw new Error(`formulas error: ${raw.error}`);
  ...
  async evaluateBatch(
    ...
    try {
      ...
      const err = e as { stderr?: Buffer };
      throw new Error(`formulas batch evaluation failed: ${err.stderr?.toString() || String(e)}`);
    }
