...
`select` also accepts arbitrary expressions — literals, arithmetic, and other constructs common in SQL — which are expanded into a column populated with the result across all rows:

```sql
select Col1, 5, 'foo', 3*1
```
...
|---|---|---|
|Any|`Col#`|The `#` refers to the column index of the input.|
|[[Range]]|`AZ`|Column letter notation. Only valid when range data is provided.|
