...
- **Imports `@cartularium/contracts`** (since the fork-annotation store, 2026-06-26) — the shared `AssayForkAnnotation*` DTO + `ALL_CAUSES`/`ALL_PLATFORMS` for request validation. This is a **runtime** edge, so build-before-consume applies: every contracts-consuming script (`dev`/`deploy`/`test`/`typecheck`) is prefixed with `pnpm --filter @cartularium/contracts run build`, and a `check` script brings edit-shell under the root `pnpm check`. Runtime deps: Hono, Octokit, valibot, @cartularium/contracts. The contracts migration has begun here (fork-annotation types now import from contracts); the remaining transitional DTOs in `src/assay-preview/config.ts` still await migration — migrate them deliberately into contracts, and don't add new local DTO copies.
